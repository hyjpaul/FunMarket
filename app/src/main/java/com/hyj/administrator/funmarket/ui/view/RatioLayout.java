...
/**
 * 自定义控件, 按照比例来决定布局高度(比如图片宽度是match_parent，高度写死的话会造成拉伸压缩等，可以让图片填充进该布局)
 */
public class RatioLayout extends FrameLayout {
    ...
    public RatioLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        // 获取属性值(布局文件中funmarket:ratio="2.43")
...
//        System.out.println("ratio:" + ratio);
    ...
    //生成布局：测量，定位，绘制(应该在测量的时候就定好布局的宽高)
    @Override
...
//        System.out.println("widthMeasureSpec:" + widthMeasureSpec);
        ...
        if (widthMode == MeasureSpec.EXACTLY && heightMode != MeasureSpec.EXACTLY && ratio > 0) {
            ...
            int imgWidth = width - getPaddingLeft() - getPaddingRight();

            // 图片高度 = 图片宽度/宽高比例
            int imgHeight = (int) (imgWidth / ratio + 0.5f);

            // 控件高度 = 图片高度 + 上侧内边距 + 下侧内边距
            height = imgHeight + getPaddingTop() + getPaddingBottom();
